...
public func <-- <D: Deserializable, T>(lhs: inout D?, rhs: T?) -> D? {
...
public func <-- <T, D: Deserializable, U>(lhs: inout [T : D]?, rhs: [T : U]?) -> [T : D]? {
...
public func <-- <T, D: Deserializable, U>(lhs: inout [T : D]?, rhs: U?) -> [T : D]? {
  guard let rhs = rhs else {
    lhs = nil
    return lhs
  }
